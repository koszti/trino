...
public class LoginClassicResource
{
            ...
            @BeanParam ExternalUriInfo externalUriInfo)
    {
        ...
        if (!formWebUiAuthenticationManager.isAuthenticationEnabled(securityContext.isSecure())) {
            return Response.seeOther(externalUriInfo.absolutePath(UI_DISABLED)).build();
        }
        ...
        if (authenticationCookie.isEmpty()) {
            ...
            return Response.seeOther(externalUriInfo.absolutePath(CLASSIC_LOGIN_FORM)).build();
        }
    ...
    public Response logout(@Context HttpHeaders httpHeaders, @Context SecurityContext securityContext, @BeanParam ExternalUriInfo externalUriInfo)
    {
        ...
        if (formWebUiAuthenticationManager.isAuthenticationEnabled(securityContext.isSecure())) {
            redirectLocation = externalUriInfo.absolutePath(CLASSIC_LOGIN_FORM);
        }
        else {
            redirectLocation = externalUriInfo.absolutePath(UI_DISABLED);
        }
